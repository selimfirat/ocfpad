...
class ConvLSTMAutoencoder(BaseDetector):
    ...
    def fit(self, X, **kwargs):
        ...
        data = [x.unsqueeze(1).cuda() for x in X]
        print(data[0].shape)
        targets = [data[i].clone().cuda() for i in range(len(data))]
...
if __name__ == "__main__":
    ...
    for i, (name, vid) in tqdm(enumerate(dev.items())):
        frame_scores = stae.predict_proba(vid["features"].unsqueeze(0))[:, 0]
        print(frame_scores.shape, frame_scores)
